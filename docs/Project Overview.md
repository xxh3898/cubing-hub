...
- 개인 기록 대시보드 / 마이페이지
  ...
  - 기록 penalty 수정 및 삭제
- 글로벌 랭킹 보드
  - 종목 필터, 닉네임 검색, 로그인 사용자 내 순위 카드, 25개 단위 페이지네이션
- 큐브 학습
  - 초보자용 8단계 단계 선택형 해법 제공
...
- 학습
  - 초보자용 8단계 목록과 단계별 완료 모습, 케이스, 해법 카드를 제공한다.
  - 1단계 십자 맞추기에서는 케이스 카드 없이 흰색 엣지를 빈 십자 자리로 옮기는 세로형 가이드와 완료 모습만 제공하고, 흰색 외 면색은 회색으로 단순화한다.
...
- 프런트 모바일 대응
  - 데스크톱은 상단 nav를 사용하고, 모바일 폭에서는 `홈`, `타이머`, `랭킹`, `학습`, `커뮤니티` 하단 tab을 핵심 이동 경로로 제공한다.
