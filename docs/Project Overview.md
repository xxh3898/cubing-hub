...
- 큐브 학습
  ...
  - 상단 첫 탭에서 WCA `3x3x3` 스크램블 기준 `U/D/L/R/F/B`의 기본, prime, double turn VisualCube 가이드 제공
- 자유 게시판
  - 목록/상세/작성/수정/삭제
  - 제목·닉네임 검색, 카테고리 필터링
- 피드백
...
- 인증
  - 백엔드 인증 API, 회원가입 이메일 인증번호 요청/확인 API, 비밀번호 재설정 인증번호 요청/확인 API, `GET /api/me`가 구현되어 있다.
  - React 로그인/회원가입/로그아웃, 비밀번호 재설정, 회원가입 이메일 인증 2단계 UI, 보호 라우트, guest-only 라우트, `401 -> refresh -> retry`가 구현되어 있다.
...
- 학습
  - `F2L 41`, `OLL 57`, `PLL 21` 정적 케이스가 구현되어 있다.
  - 학습 화면 상단 첫 탭에서 WCA `3x3x3` 스크램블 기준 `U/D/L/R/F/B`의 기본, prime, double turn VisualCube 가이드를 제공한다.
- 프런트 모바일 대응
...
- 마이페이지 / 피드백
  - 마이페이지는 프로필/요약, 계정 관리 모달 기반 닉네임/주 종목 수정과 현재 비밀번호 기반 비밀번호 변경, 주 종목 기준 최근 기록 추세 그래프, 전체 기록 페이지 조회, 기록 penalty 수정/삭제가 연동되어 있다.
  - 홈 대시보드는 `GET /api/home` 기준으로 연동되어 있다.
  - 피드백은 `POST /api/feedbacks` 기준으로 로그인 사용자 제출, Discord 운영 알림 상태 확인, 실패 시 재시도 흐름까지 연동한다.
  - 공개된 질문/답변은 `/qna` 기준 public 읽기 화면으로 노출하고, 관리자 전용 `/admin`에서 피드백 답변/공개 전환과 내부 메모를 함께 관리한다.
- 커뮤니티 운영
  - 게시글은 다중 이미지 첨부를 지원하고 첨부 이미지는 S3 + DB metadata 기준으로 관리한다.
