...
| --- | --- | --- | --- | --- | --- | --- |
| V2.1 Timer / Record Foundation | Practice solve의 시간·입력·저장·조회 계약 안정화 | 이후 Growth와 Participation이 신뢰할 Record 기반 필요 | current Timer/Record 조사, product decision, production event audit | canonical time, input provenance, idempotency, pending solve, history·average, event capability, migration test gate | Session, Daily Challenge, device 연결, verification, competition | [PRD](prd.md)의 V2.1 acceptance 충족과 event capability 결정 |
