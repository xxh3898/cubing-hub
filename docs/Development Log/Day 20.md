...
- Day 19와 같은 `300,000` PB seed, 같은 `k6` 시나리오로 V2 재측정을 실행하고 전/후 비교 산출물을 생성
- Day 20 결과에 맞춰 일정, 설계 문서, 개발 로그 허브를 동기화
...
#### 문제 상황
- Day 19 baseline 기준 `GET /api/rankings?eventType=WCA_333&page=1&size=25`는 `300,000` PB 후보에서 `p95 13.02s`가 나왔다.
...
- Redis ZSET score는 `bestTimeMs`, member는 `createdAtMillis:recordId:userId` zero-padded 문자열로 구성해 기존 tie-break 계약인 `created_at asc -> record.id asc`를 유지했다.
...
#### 문제 상황
- Day 20 원래 계획에는 최종 배포가 포함돼 있었지만, 저장소 안에는 프로덕션 env 값, 실제 도메인, AWS 자원, 배포 스크립트가 충분히 정리돼 있지 않았다.
