...
- 구현 상태
  ...
  - 오늘의 스크램블과 최근 기록의 실제 API 연동은 미구현 상태다.
...
- 구현 상태
  ...
  - 제출은 목업 처리이며 실제 게시글 생성 API 연동은 미구현 상태다.
...
- 사용자 액션
  ...
  - 목록으로 복귀
- 구현 상태
  - 데이터 소스는 `mockCommunityPosts`와 로컬 state다.
  - 비로그인 상태에서는 댓글 입력 대신 로그인 CTA를 노출한다.
  - 댓글 API와 실제 게시글 상세/삭제 연동은 미구현 상태다.
...
- 구현 상태
  - 로컬 폼 처리만 구현되어 있다.
  - 관리자 메일 전달 및 백엔드 저장/전송 로직은 미구현 상태다.
