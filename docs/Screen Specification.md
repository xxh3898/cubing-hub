...
- 기준 원칙:
  ...
  - 공통 앱 shell은 데스크톱 상단 nav와 모바일 하단 tab을 함께 사용한다. 모바일 하단 tab의 핵심 경로는 `홈`, `타이머`, `랭킹`, `학습`, `커뮤니티`다.
...
| 랭킹 | `/rankings` | 포디움, 내 순위 카드, 종목 필터, 닉네임 검색, 25개 단위 페이지네이션 | `GET /api/rankings` 연동 완료 |
| 학습 | `/learning` | 초보자 8단계 단계 선택형 해법, F2L/OLL/PLL 119 케이스 조회, 회전기호 가이드 | 정적 학습 데이터 렌더링 구현 |
...
- 구현 상태
  ...
  - 기본 랭킹 화면(`nickname` 검색 없음, 1페이지)에서만 API 결과의 상위 3명을 포디움으로 함께 보여준다.
  - 로그인 사용자는 응답의 `myRanking` 기준으로 선택 종목의 내 순위를 확인할 수 있고, 해당 종목 PB가 없으면 내 순위 empty 상태를 보여준다.
  - 페이지네이션은 `1~10` 단위 그룹과 `이전`, `다음`, `<<`, `>>` 이동으로 동작한다.
  - 서버 검색, 서버 페이지네이션, `loading`, `empty`, `error`, 재시도 상태가 반영되어 있다.
  - 모바일 폭에서는 포디움과 검색/종목 툴바가 세로 배치로, 랭킹 데이터는 카드형 행으로 재배치된다.

### 학습

- 레이아웃 구조
  - 학습 소개 헤더와 콘텐츠 요약
  - `회전기호 가이드`, `초보자`, `F2L`, `OLL`, `PLL` 탭
  - `초보자` 탭의 8단계 목록
  - 케이스 카드 그리드
- 주요 UI 요소
  - 초보자 8단계, CFOP 119 케이스, 회전기호 18개 요약
  - 초보자 단계 선택 카드
  - 1단계 십자 맞추기 세로형 가이드
  - 초보자 단계별 완료 모습 카드
  - 초보자 단계 상세 상단의 다음 단계 이동 버튼
  - 케이스 이미지
  - 회전기호 VisualCube 이미지
  - 알고리즘 텍스트와 단계별 설명
  - 표기법 예시 코드 블록
- 화면 데이터 요구사항
  ...
  - 초보자 단계별 완료 모습
  - 초보자 단계별 케이스와 해법
  - 1단계 십자 맞추기 기준 잡는 방향 안내, 단계별 가이드, 완료 모습
  - 정적 학습 데이터 119건
...
- 사용자 액션
  - 탭 전환
  - 초보자 단계 선택과 단계 목록 복귀
  - 초보자 단계 상세에서 다음 단계로 이동
  - 케이스 탐색
- 구현 상태
  - 정적 데이터 기반 렌더링이 구현되어 있다.
  - `초보자` 탭에서 3x3x3 초보자 해법 8단계 목록을 먼저 보여주고, 단계를 선택하면 해당 단계의 완료 모습, 케이스, 해법 카드를 보여준다.
  - 초보자 단계 상세 상단에는 `단계 목록`과 다음 단계가 있을 때 `다음 단계 (n단계: 제목)` 버튼을 먼저 표시하고, 그 아래에 현재 단계 제목과 설명을 표시한다. 8단계에서는 다음 단계 버튼을 표시하지 않는다.
  - 모바일 폭에서는 학습 탭을 가로 스크롤 가능한 segmented control로 표시하고, 단계/케이스/가이드 카드는 1열 중심으로 재배치된다.
  - 1단계 `십자 맞추기`는 케이스 카드와 케이스 이미지를 보여주지 않고, 자체 VisualCube 기반 세로형 가이드와 완료 모습만 보여준다. 흰색 조각 식별에 집중할 수 있도록 흰색 외 facelet은 회색으로 단순화하고, 회전기호 가이드와 같은 기본 3D 각도로 렌더링한다.
  - 2단계 `십자 엣지 맞추기`는 흰색 십자를 상단에 둔 `fc` facelet 그림으로 완료 모습과 인접/반대 엣지 보정 케이스를 구분한다. 완료 모습과 케이스는 기본 3D 각도로 렌더링하고, 인접/반대 케이스는 옆면 색이 회색으로 마스킹되지 않도록 VisualCube `stage`를 지정하지 않는다.
  - 3단계 `1층 맞추기`는 공식 기반 케이스 3개만 제공하고, `잘못 들어간 코너 빼내기` 케이스는 초보자 화면에서 제외한다. 3층 엣지 스티커는 보이는 옆면까지 회색으로 가리고, 코너 색과 1,2층 색은 유지한다.
  - 4단계 `2층 맞추기`는 오른쪽/왼쪽 2층 삽입 케이스 2개만 제공하고, `잘못 들어간 엣지 빼내기` 케이스는 초보자 화면에서 제외한다. 2층 완료 모습은 기본 3D 각도로 렌더링한다.
  - 6단계 `윗면 맞추기`는 윗면 십자를 유지한 미완성 코너 방향 7가지를 `fc` facelet으로 직접 지정한다. 각 윗층 코너 조각은 노란 스티커가 윗면 또는 인접 옆면 중 한 곳에만 보이도록 구성하고, 보이는 옆면 코너 스티커도 노란색으로 표시한다. 케이스별로 공식 `R' F' L' F R F' L F`와 필요한 `U`, `U'`, `U2` 기준 조정을 순서대로 제공한다.
  - 7단계 `3층 코너 맞추기`는 윗면을 모두 노란색으로 표시하고, 옆면의 3층 엣지 조각만 회색으로 가린다. 1층과 2층의 맞춰진 색은 유지한다. 인접 코너 교환과 대각선 코너 교환은 `fc` facelet으로 공식 적용 전 상태를 직접 지정하고, 대각선 교환은 전용 대각선 교환 공식을 사용한다.
  - 초보자 케이스 카드의 표시용 해법과 VisualCube 그림용 상태값을 분리해, 같은 공식을 반복하는 케이스도 서로 다른 그림으로 구분한다.
  - 1단계 `십자 맞추기`는 목표 확인, 흰색 센터 기준 잡기, 흰색 엣지 찾기, 빈 십자 자리로 옮기기, 반복하기 흐름을 안내한다.
...
- 구현 상태
  ...
  - 화면 헤더는 `피드백 보내기` 문구와 회신 이메일 안내를 사용한다.
...
- 레이아웃 구조
  - 목록: Q&A 안내 헤더, 공개 답변 요약, 질문 카드 목록, 페이지네이션
...
- 구현 상태
  ...
  - 목록 화면은 공개 답변 수와 피드백 이동 CTA를 함께 보여준다.
  - 모바일 폭에서는 Q&A 헤더 요약과 카드가 1열로 재배치된다.

### 관리자 메인 / 상세

- 레이아웃 구조
  - 메인: 운영 관리 헤더, 피드백 탭, 관리자 메모 탭, 피드백 필터, 메모 생성 폼
...
- 구현 상태
  - `/admin`은 관리자 전용 라우트다.
  - 관리자 메인은 운영 도구 성격에 맞춰 조밀한 헤더와 상태 요약을 사용한다.
