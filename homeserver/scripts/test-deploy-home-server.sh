...
/usr/bin/sed \
  ...
  -e "s#^WEB_IMAGE=.*#WEB_IMAGE=ghcr.io/xxh3898/cubing-hub-web:${REVISION_ONE}#" \
...
set -e
if [[ "${db_entrypoint_override_exit_code}" -ne 1 ]]; then
  printf 'Runtime config with a MySQL entrypoint override must fail\n' >&2
  exit 1
fi
...
printf 'Cubing Hub deploy v2 tests passed\n'
